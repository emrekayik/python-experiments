...
def connection():
    try:
        conn=sqlite3.connect("kutubhane.db")
    except:
        print("veritabanına bağlanamıyor")
    return conn    
...
def verifier():
    ...
    if not student_name.get():
        t1.insert(END,"<>Öğrenci Adı Gerekli<>\n")
        a=1
    if not roll_no.get():
        t1.insert(END,"<>Sınıf  Numarası Gerekli<>\n")
        b=1
    if not branch.get():
        t1.insert(END,"<>Alanı Gerekli<>\n")
        c=1
    if not phone.get():
        t1.insert(END,"<>Telefon Numarası Gerekli<>\n")
        d=1
    if not father.get():
        t1.insert(END,"<>Baba Adı Gerekli<>\n")
        e=1
    if not address.get():
        t1.insert(END,"<>Adres Gerekli<>\n")
        f=1
...
if __name__=="__main__":
    ...
    label1=Label(root,text="Öğrenci Adı:")
